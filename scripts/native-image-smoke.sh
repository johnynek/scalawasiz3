...
printf '%s' "$SAT_INPUT" | "$BIN" >"$TMP_DIR/sat.out" 2>"$TMP_DIR/sat.err"
if [[ $? -ne 0 ]]; then
  echo "SAT example exited non-zero" >&2
...
printf '%s' "$UNSAT_INPUT" | "$BIN" >"$TMP_DIR/unsat.out" 2>"$TMP_DIR/unsat.err"
if [[ $? -ne 0 ]]; then
  echo "UNSAT example exited non-zero" >&2
